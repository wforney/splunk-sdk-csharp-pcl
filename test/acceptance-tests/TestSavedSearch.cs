...
public class SavedSearchTest
{
    ...
    public async Task SavedSearchDispatchProperties()
    {
        ...
        var name = "sdk-test_DispatchProperties";
        var testSearch = await savedSearches.GetOrNullAsync(name);

        if (testSearch is not null)
        {
        ...
        var job = await testSearch.DispatchAsync();
    ...
    public async Task SavedSearchesProperties()
    {
        ...
        foreach (var savedSearch in savedSearches)
        {
            ...
            var emailFormat = savedSearch.Actions.Email.Format;
            ...
            var paperOrientation = savedSearch.Actions.Email.ReportPaperOrientation;
            var paperSize = savedSearch.Actions.Email.ReportPaperSize;
            ...
            var scriptT = savedSearch.Actions.Script;
            ...
            var alertSeverity = savedSearch.Alert.Severity;
            var suppress = savedSearch.Alert.Suppress;
            dummyString = suppress.Fields;
            dummyString = suppress.Period;
            var track = savedSearch.Alert.Track;
            var comparator = savedSearch.Alert.Comparator;
            dummyString = savedSearch.Alert.Condition;
            dummyString = savedSearch.Alert.Threshold;
            var alertType = savedSearch.Alert.AlertType;
            ...
            var display = savedSearch.Display;
        ...
        for (var i = 0; i < savedSearches.Count; i++)
        {
            ...
            dummyBool = savedSearch.Actions.Rss.TrackAlert;
            dummyString = savedSearch.Actions.Rss.Ttl;
            var script = savedSearch.Actions.Script;
    ...
    public async Task SavedSearchesUpdateProperties()
    {
        ...
        Assert.NotNull(testSearch);
        var updatedSnapshot = await testSearch!.UpdateAsync(new SavedSearchAttributes() { IsVisible = false });
        ...
        _ = await testSearch.UpdateAsync(attributes);
    ...
    public async Task SavedSearchHistory()
    {
        ...
        var savedSearch = savedSearches.SingleOrDefault(ss => ss.Name == name);
        ...
        var history = await savedSearch.GetHistoryAsync();
        ...
        Assert.Contains(history, a => a.Sid == job1.Sid); // this.Contains(history, job1.Sid));

        var job2 = await savedSearch.DispatchAsync();
        history = await savedSearch.GetHistoryAsync();
        Assert.Equal(2, history.Count);
        Assert.Contains(history, a => a.Sid == job1.Sid);
        Assert.Contains(history, a => a.Sid == job2.Sid);
