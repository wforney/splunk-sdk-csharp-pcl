...
public class ConfigurationsTest
{
    ...
    public async Task Configuration()
    {
        ...
        using (var service = await SdkHelper.CreateService())
        {
            _ = await service.Applications.RecreateAsync(app);
        ...
        using (var service = await SdkHelper.CreateService(new Namespace(user: "nobody", app: app)))
        {
            ...
            Assert.DoesNotContain(confs, a => a.Name == "testconf");

            var testconf = await confs.CreateAsync("testconf");
            testconf = await confs.GetOrNullAsync("testconf");
            Assert.NotNull(testconf);

            var stanza1 = await testconf!.CreateAsync("stanza1");
            var stanza2 = await testconf.CreateAsync("stanza2");
            var stanza3 = await testconf.CreateAsync("stanza3");
            ...
            var updatedSnapshot = await stanza1.UpdateAsync(new Argument("key1", "value2"));
